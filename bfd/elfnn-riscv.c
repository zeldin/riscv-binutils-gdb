...
#define riscv_get_i16 bfd_get_16
#define riscv_put_i16 bfd_put_16
static bfd_vma
riscv_get_i32 (bfd *abfd, const void *p)
{
  return bfd_get_16(abfd, p) |
    (bfd_get_16(abfd, ((const bfd_byte *)p)+2) << 16);
}
static void
riscv_put_i32 (bfd *abfd, bfd_vma data, void *p)
{
  bfd_put_16(abfd, data, p);
  bfd_put_16(abfd, data >> 16, ((bfd_byte *)p)+2);
}
static bfd_vma
riscv_get_i64 (bfd *abfd, const void *p)
{
  return bfd_get_16(abfd, p) |
    (bfd_get_16(abfd, ((const bfd_byte *)p)+2) << 16) |
    (bfd_get_16(abfd, ((const bfd_byte *)p)+4) << 32) |
    (bfd_get_16(abfd, ((const bfd_byte *)p)+6) << 48);
}
static void
riscv_put_i64 (bfd *abfd, bfd_vma data, void *p)
{
  bfd_put_16(abfd, data, p);
  bfd_put_16(abfd, data >> 16, ((bfd_byte *)p)+2);
  bfd_put_16(abfd, data >> 32, ((bfd_byte *)p)+4);
  bfd_put_16(abfd, data >> 48, ((bfd_byte *)p)+6);
}
#define riscv_get_insn(bits, abfd, ptr)			\
  ((bits) == 16 ? riscv_get_i16 (abfd, ptr)             \
   : (bits) == 32 ? riscv_get_i32 (abfd, ptr)		\
   : (bits) == 64 ? riscv_get_i64 (abfd, ptr)		\
   : (abort (), (bfd_vma) - 1))
#define riscv_put_insn(bits, abfd, val, ptr)		\
  ((bits) == 16 ? riscv_put_i16 (abfd, val, ptr)        \
   : (bits) == 32 ? riscv_put_i32 (abfd, val, ptr)	\
   : (bits) == 64 ? riscv_put_i64 (abfd, val, ptr)	\
   : (abort (), (void) 0))
	...
	case R_RISCV_CALL_PLT:
	  ...
	  if (h != NULL && h->root.type == bfd_link_hash_undefweak
	      && (!bfd_link_pic (info) || h->plt.offset == MINUS_ONE))
	    {
	      ...
	      bfd_vma insn = riscv_get_i32 (input_bfd,
					    contents + rel->r_offset + 4);
	      insn &= ~(OP_MASK_RS1 << OP_SH_RS1);
	      riscv_put_i32 (input_bfd, insn, contents + rel->r_offset + 4);
	  ...
	  if (VALID_ITYPE_IMM (relocation + rel->r_addend))
	    {
	      ...
	      bfd_vma insn = riscv_get_i32 (input_bfd, contents + rel->r_offset);
	      insn &= ~(OP_MASK_RS1 << OP_SH_RS1);
	      insn |= X_TP << OP_SH_RS1;
	      riscv_put_i32 (input_bfd, insn, contents + rel->r_offset);
	    }
	  else
	    r = bfd_reloc_overflow;
	  break;
				 ...
				 Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
	...
	riscv_put_i32 (output_bfd, plt_entry[i], loc + 4*i);
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (splt->size > 0)
	{
	  ...
	  for (i = 0; i < PLT_HEADER_INSNS; i++)
	    riscv_put_i32 (output_bfd, plt_header[i], splt->contents + 4*i);
		       ...
		       bfd_boolean undefined_weak ATTRIBUTE_UNUSED)
{
  ...
  auipc = riscv_get_i32 (abfd, contents + rel->r_offset);
  jalr = riscv_get_i32 (abfd, contents + rel->r_offset + 4);
		      ...
		      bfd_boolean undefined_weak)
{
	  ...
	  if (undefined_weak)
	    {
	      ...
	      bfd_vma insn = riscv_get_i32 (abfd, contents + rel->r_offset);
	      insn &= ~(OP_MASK_RS1 << OP_SH_RS1);
	      riscv_put_i32 (abfd, insn, contents + rel->r_offset);
	    }
	  else
  ...
  if (use_rvc
			       ...
			       : ELF_MAXPAGESIZE)))
    {
      ...
      bfd_vma lui = riscv_get_i32 (abfd, contents + rel->r_offset);
      unsigned rd = ((unsigned)lui >> OP_SH_RD) & OP_MASK_RD;
      if (rd == 0 || rd == X_SP)
	return TRUE;

      lui = (lui & (OP_MASK_RD << OP_SH_RD)) | MATCH_C_LUI;
      riscv_put_i32 (abfd, lui, contents + rel->r_offset);
			...
			bfd_boolean undefined_weak ATTRIBUTE_UNUSED)
{
  ...
  for (pos = 0; pos < (nop_bytes & -4); pos += 4)
    riscv_put_i32 (abfd, RISCV_NOP, contents + rel->r_offset + pos);

  /* Write a final RVC NOP if need be.  */
  if (nop_bytes % 4 != 0)
    riscv_put_i16 (abfd, RVC_NOP, contents + rel->r_offset + pos);
